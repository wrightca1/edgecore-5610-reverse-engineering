...
|------|-----|------|------|-----|
| **Live switch (Cumulus)** | 10.1.1.233 | cumulus | Running Cumulus Linux, switchd, BDE. For strace, packet I/O traces, port-up traces. | Cumulus Linux |
| **Ghidra server** | 10.1.1.30 | <USER> | Ghidra 12, JDK 21, analyzeHeadless. Binaries and Ghidra projects live here. | Fedora (30GB RAM) |
| **Debian build server (modern)** | 10.22.1.4 | <USER> | General builds | Debian |
| **Debian build server (Debian 8)** | 10.22.1.5 | <USER> | Legacy/ONL builds | Debian 8 |
| **ONL switch** | 10.1.1.222 | root | ONL / bare-metal switch (different from Cumulus) | ONL |

---

## Paths

### Ghidra server (10.1.1.30)
...
**Disk**: Root/home often has little space. Use `./run-switch-disk-cleanup-10.1.1.233.sh` to report usage; `--clean` to remove old trace files.
...
- `run-configure-ospf-101-10.1.1.233.sh` + `configure-ospf-101-on-switch.sh` — OSPF on swp1 and swp2 (10.101.101.0/29 and 10.101.101.8/29 area 0)
...
|----------|---------|-----|
| `GHIDRA_HOST` | <USER>@10.1.1.30 | Ghidra server |
| `REMOTE_PROJECT` | /home/<USER>/analysis | Base on Ghidra server |
| `SWITCH` | cumulus@10.1.1.233 | Live Cumulus switch |
...
1. **Traces on live switch**: Scripts SSH to `cumulus@10.1.1.233`, run strace/trace scripts, SCP results to `docs/reverse-engineering/traces/`.
2. **Ghidra on server**: Scripts SSH to `<USER>@10.1.1.30`, run analyzeHeadless with `-scriptPath` and `-postScript`. Outputs go to `build-server/*/sdk-deep-extract/`.
3. **Copy outputs locally**: `scp <USER>@10.1.1.30:/home/<USER>/analysis/build-server/.../sdk-deep-extract/FILE build-server/.../sdk-deep-extract/`
...
scp <USER>@10.1.1.30:/home/<USER>/analysis/build-server/opennsl/sdk-deep-extract/*.txt build-server/opennsl/sdk-deep-extract/

# From Ghidra server, switchd outputs
scp <USER>@10.1.1.30:/home/<USER>/analysis/build-server/switchd/sdk-deep-extract/*.txt build-server/switchd/sdk-deep-extract/

# From live switch, traces
scp cumulus@10.1.1.233:/home/cumulus/port_up_switchd.log docs/reverse-engineering/traces/port-up-trace-10.1.1.233.log
```
...
|--------|---------|-------------|
| `run-dump-func-on-build-server.sh` | Local | Ghidra server (10.1.1.30) |
...
| `run-port-up-trace-10.1.1.233.sh` | Local | Live switch (10.1.1.233) |
| `run-perf-port-up-10.1.1.233.sh` | Local | Live switch |
| `run-bar-diff-port-up-10.1.1.233.sh` | Local | Live switch (dumps /dev/mem before/after port up) |
| `run-packet-io-trace-10.1.1.233.sh` | Local | Live switch |
| `run-bde-mmap-trace-10.1.1.233.sh` | Local | Live switch |
