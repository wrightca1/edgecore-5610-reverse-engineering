...
| **VLAN tables** | **HAVE** | **VLAN (ingress) + EGR_VLAN (egress) fully verified via bcmcmd on live switch (2026-02-23)**. VLAN: 0x12168000, 4096×40B, direct-indexed by VLAN ID. PORT_BITMAP@[65:0] (CPU=bit0, xe0=bit1, ..., xe51=bit52), ING_PORT_BITMAP@[131:66], STG@[140:132], VALID@205, VLAN_PROFILE_PTR@[238:232]. EGR_VLAN: 0x0d260000, 4096×29B. VALID@0, STG@[9:1], UT_PORT_BITMAP@[161:96] (untagged ports), PORT_BITMAP@[227:162]. Port-to-bit: xe0→bit1, xe1→bit2, ..., xe51→bit52, CPU→bit0. [VLAN_TABLE_FORMAT.md](VLAN_TABLE_FORMAT.md) |
| **Packet I/O (4.x)** | **HAVE** | **DCB format verified via bcmcmd dma dcbdump on live switch (2026-02-24)**. DCB type 21, 16 words (64 bytes) per DCB. TX: word[0]=buf_phys, word[1][15:0]=pkt_len, word[2]=0xff000000 (START), word[3]=0x00000100 (UNICAST=1), word[4][7:0]=LOCAL_DEST_PORT (1=xe0, 2=xe1, ..., 52=xe51), word[15]=status. RX: same format, ASIC fills words[2..14] with metadata, word[15]=done+pkt_len. BDE ioctls: LUBDE_WAIT_FOR_INTERRUPT (0x20004c09), LUBDE_SEM_OP (0x20004c0a) for sync only — packets flow via mmap'd DMA rings, no packet ioctl. DMA channels: TX=ch0, RX=ch1..3. DMA pool: 0x04000000 phys (64MB). [PKTIO_BDE_DMA_INTERFACE.md](PKTIO_BDE_DMA_INTERFACE.md), [PACKET_IO_VERIFIED.md](PACKET_IO_VERIFIED.md) |
...
- [VLAN_TABLE_FORMAT.md](VLAN_TABLE_FORMAT.md) - **VLAN (ingress) + EGR_VLAN (egress) field layouts — direct-indexed by VLAN ID, PORT_BITMAP + ING_PORT_BITMAP + UT_PORT_BITMAP bit positions verified**
- [PKTIO_BDE_DMA_INTERFACE.md](PKTIO_BDE_DMA_INTERFACE.md) - **Packet I/O BDE DMA interface — DCB type 21, 16 words, TX/RX word layout, LOCAL_DEST_PORT encoding, BDE ioctls, thread model all verified**
- [STATS_COUNTER_FORMAT.md](STATS_COUNTER_FORMAT.md) - **Stats counter register map — XLMAC register offsets (RPKT/RBYT/TPKT/TBYT/R64/T64/etc), S-Channel address formula, port→block/lane mapping — all verified via listreg/getreg**
