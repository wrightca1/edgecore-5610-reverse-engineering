# Packet I/O trace analysis — <LIVE_SWITCH_IP>

**Switch**: <SWITCH_USER>@<LIVE_SWITCH_IP>  
**Run**: Via `run-packet-io-trace-<LIVE_SWITCH_IP>.sh` with `SSHPASS` and sudo password piped.

## Result (updated)

- **Trace file**: `packet-io-trace-<LIVE_SWITCH_IP>.log`
- **Summary**: `packet-io-summary-<LIVE_SWITCH_IP>.txt`
...
**Full extraction** (thread roles, fd map, BDE ioctl decode, TX/RX sequences, raw samples): [PACKET_IO_TRACE_EXTRACTED_<LIVE_SWITCH_IP>.md](PACKET_IO_TRACE_EXTRACTED_<LIVE_SWITCH_IP>.md).
...
2. **Run the trace** (from your machine, with SSH auth):
   ```bash
   SSHPASS='yourpassword' ./scripts/reverse-engineering/run-packet-io-trace-<LIVE_SWITCH_IP>.sh 10.1.1.1
   ```

3. **If the main trace still has only 2 lines**, run a **probe** to confirm strace can see any syscalls (5s of all syscalls):
   ```bash
   DO_PROBE=1 SSHPASS='yourpassword' ./scripts/reverse-engineering/run-packet-io-trace-<LIVE_SWITCH_IP>.sh 10.1.1.1
   ```
   Then check `traces/packet-io-probe-<LIVE_SWITCH_IP>.log`. If the probe has many lines, strace works and the main trace filter or timing is the issue. If the probe also has only 2 lines, check **ptrace_scope** (script prints it) and disk space.
...
ssh <SWITCH_USER>@<LIVE_SWITCH_IP> 'cat /proc/sys/kernel/yama/ptrace_scope'
```
...
| File | Description |
|------|-------------|
| `packet-io-trace-<LIVE_SWITCH_IP>.log` | Raw strace output (read/write/ioctl only). |
| `packet-io-summary-<LIVE_SWITCH_IP>.txt` | Extracted reads, writes, ioctls + first 80 lines (empty in this run). |
