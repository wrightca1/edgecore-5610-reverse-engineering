# Trace run summary — <LIVE_SWITCH_IP>

**Date**: 2026-02-19

## Setup

- **Switch**: <LIVE_SWITCH_IP> (Cumulus), user `cumulus`
...
- `./scripts/reverse-engineering/run-configure-ospf-101-<LIVE_SWITCH_IP>.sh`

## Traces run

### 1. Route/ECMP strace (L3 + ECMP API activity)

- **Script**: `run-route-ecmp-strace-<LIVE_SWITCH_IP>.sh` → runs `trace-route-ecmp-strace-on-switch.sh` on switch
...
- **Output**: 
  - `docs/reverse-engineering/traces/route-ecmp-strace-<LIVE_SWITCH_IP>.log` (7,246 lines in latest run)
  - `docs/reverse-engineering/traces/route-ecmp-strace-summary-<LIVE_SWITCH_IP>.txt`
- **Content**: switchd `ioctl(14, 0x20004c09)` / `0x20004c0a` (BDE), `mmap` during route/ECMP programming

### 2. OSPF strace

- **Script**: `run-ospf-strace-<LIVE_SWITCH_IP>.sh` → runs `trace-ospf-strace-on-switch.sh` on switch
- **Trigger**: `vtysh -c "clear ip ospf neighbor"` (re-adjacency and FIB refresh while tracing switchd)
- **Output**:
  - `docs/reverse-engineering/traces/ospf-strace-<LIVE_SWITCH_IP>.log` (17,044 lines in run)
  - `docs/reverse-engineering/traces/ospf-strace-summary-<LIVE_SWITCH_IP>.txt`
- **Content**: switchd BDE ioctl (0x20004c09, 0x20004c0a), mmap, write during OSPF-driven FIB updates

### 3. Port-up strace (swp2)

- **Script**: `run-port-up-trace-<LIVE_SWITCH_IP>.sh swp2`
- **Trigger**: `ip link set swp2 down` then `up`
- **Output**:
  - `docs/reverse-engineering/traces/port-up-trace-<LIVE_SWITCH_IP>.log` (12,295 lines)
  - `docs/reverse-engineering/traces/port-up-summary-<LIVE_SWITCH_IP>.txt`

### 4. API-to-table-ID (GDB) trace

- **Script**: `run-api-to-table-id-trace-<LIVE_SWITCH_IP>.sh`
